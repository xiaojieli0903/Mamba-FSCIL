...
def build_dataset(cfg: Dict, default_args: Optional[Dict] = None) -> Dataset:
    if isinstance(cfg, (list, tuple)):
        ...
        dataset = EpisodicDataset(build_dataset(cfg['dataset'], default_args),
        ...
        dataset = MetaTestDataset(build_dataset(cfg['dataset'], default_args),
                                  num_episodes=cfg['num_episodes'],
                                  num_ways=cfg['num_ways'],
                                  num_shots=cfg['num_shots'],
                                  num_queries=cfg['num_queries'],
...
def build_meta_test_dataloader(dataset: Dataset, meta_test_cfg: Dict,
    ...
    support_data_loader = DataLoader(
        ...
        **kwargs)
    query_data_loader = DataLoader(copy.deepcopy(dataset).query(),
                                   batch_size=query_batch_size,
                                   num_workers=num_query_workers,
                                   collate_fn=partial(
                                       collate,
                                       samples_per_gpu=query_batch_size),
                                   pin_memory=False,
                                   shuffle=False,
                                   **kwargs)
    ...
    if meta_test_cfg.get('fast_test', False):
        ...
        test_set_data_loader = DataLoader(copy.deepcopy(dataset).test_set(),
                                          batch_size=all_batch_size,
                                          num_workers=num_all_workers,
                                          collate_fn=partial(
                                              collate,
                                              samples_per_gpu=all_batch_size),
                                          pin_memory=False,
                                          shuffle=False,
                                          **kwargs)
    else:
