...
def single_gpu_meta_test(model: Union[MMDataParallel, nn.Module],
    ...
    for task_id in range(num_test_tasks):
        ...
        eval_result = query_dataloader.dataset.evaluate(results,
                                                        gt_labels,
                                                        logger=logger,
                                                        **eval_kwargs)
...
def multi_gpu_meta_test(model: MMDistributedDataParallel,
    ...
    collect_results_list = collect_results_cpu(results_list,
                                               num_test_tasks,
                                               tmpdir=None)
    if rank == 0:
        if show_task_results:
        ...
        meta_eval_results = dict()
        print_log(f'number of tasks: {len(collect_results_list)}',
                  logger=logger)
        # get the average accuracy and std
