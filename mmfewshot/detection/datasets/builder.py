...
def build_dataset(cfg: ConfigDict,
    ...
    if isinstance(cfg, (list, tuple)):
        ...
        dataset = QueryAwareDataset(query_dataset,
                                    support_dataset,
        ...
        auxiliary_dataset = build_dataset(cfg['auxiliary_dataset'],
                                          default_args)
        dataset = TwoBranchDataset(main_dataset=main_dataset,
                                   auxiliary_dataset=auxiliary_dataset,
                                   reweight_dataset=cfg.get(
...
def build_dataloader(dataset: Dataset,
    ...
    (sampler, batch_size,
     num_workers) = build_sampler(dist=dist,
                                  shuffle=shuffle,
                                  dataset=dataset,
                                  num_gpus=num_gpus,
    ...
    if isinstance(dataset, QueryAwareDataset):
        ...
        query_data_loader = DataLoader(dataset,
                                       batch_size=batch_size,
                                       sampler=sampler,
                                       num_workers=num_workers,
                                       collate_fn=partial(
        ...
        support_data_loader = DataLoader(support_dataset,
                                         batch_size=1,
        ...
        main_data_loader = DataLoader(dataset,
                                      batch_size=batch_size,
                                      sampler=sampler,
                                      num_workers=num_workers,
                                      collate_fn=partial(
        ...
        auxiliary_data_loader = DataLoader(
            ...
            collate_fn=partial(multi_pipeline_collate_fn,
                               samples_per_gpu=auxiliary_samples_per_gpu),
            pin_memory=False,
            worker_init_fn=init_fn,
            **kwargs)
...
def build_sampler(
    ...
    if dist:
        ...
        if shuffle:
            if use_infinite_sampler:
                sampler = DistributedInfiniteGroupSampler(dataset,
                                                          samples_per_gpu,
                                                          world_size,
                                                          rank,
                                                          seed=seed)
            ...
            if use_infinite_sampler:
                sampler = DistributedInfiniteSampler(dataset,
                                                     world_size,
                                                     rank,
                                                     shuffle=False,
                                                     seed=seed)
        ...
        if use_infinite_sampler:
            sampler = InfiniteGroupSampler(dataset,
                                           samples_per_gpu,
                                           seed=seed,
                                           shuffle=shuffle)
        else:
