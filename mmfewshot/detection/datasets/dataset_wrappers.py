...
class QueryAwareDataset:
    ...
    def __init__(self,
        ...
        if hasattr(self.query_dataset, 'flag'):
            self.flag = np.zeros(len(self.query_dataset) * self.repeat_times,
                                 dtype=np.uint8)
    ...
    def __getitem__(self, idx: int) -> Dict:
        ...
        support_class = np.random.choice(support_class,
                                         min(self.num_support_ways - 1,
                                             len(support_class)),
                                         replace=False)
    ...
    def sample_support_shots(
        ...
        while len(support_idxes) < self.num_support_shots:
            selected_gt_idxes = np.random.choice(num_total_shots,
                                                 num_sample_shots,
                                                 replace=False)
...
class NWayKShotDataset:
    ...
    def generate_support_batch_indices(
        ...
        for _ in range(dataset_len):
            batch_indices = []
            selected_classes = np.random.choice(len(self.CLASSES),
                                                self.num_support_ways,
                                                replace=False)
            for cls in selected_classes:
                num_gts = len(self.data_infos_by_class[cls])
                selected_gts_idx = np.random.choice(num_gts,
    ...
    def save_support_data_infos(self, support_output_path: str) -> None:
        ...
        with open(support_output_path, 'w', encoding='utf-8') as f:
            json.dump(meta_info + support_data_infos,
                      f,
                      ensure_ascii=False,
                      indent=4,
                      cls=NumpyEncoder)
