...
class MultiRelationBBoxHead(BBoxHead):
    ...
    def __init__(self,
        ...
        if self.patch_relation:
            self.patch_relation_branch = nn.Sequential(
                ...
                # 5x5 -> 3x3
                nn.Conv2d(int(self.in_channels / 4),
                          int(self.in_channels / 4),
                          3,
                          padding=0,
                          bias=False),
    ...
    def forward(self, query_feat: Tensor,
        ...
        if self.local_correlation:
            ...
            local_feat = F.conv2d(local_query_feat,
    ...
    def loss(
        ...
        fg_samples_inds = torch.nonzero(labels == 0,
                                        as_tuple=False).squeeze(-1)
        bg_samples_inds = torch.nonzero(labels == 1,
                                        as_tuple=False).squeeze(-1)
