...
def modify_logit_for_repetition_penalty(logits,
                                        prev_output_tokens,
    ...
    if top_k == 1:  # Short-circuit for greedy decoding
        ...
        if top_k > 0:
            ...
            return indices[
                ...
                torch.multinomial(torch.softmax(logits_top, dim=-1),
            ...
            if min_p > 0.0:
                ...
                if temperature != 1.0:
                    logits_top /= temperature
                return torch.multinomial(torch.softmax(logits_top, dim=-1),
                                         num_samples=1).squeeze(dim=-1)
...
def decode(input_ids,
    ...
    teacher_output_len = teacher_outputs.shape[
        1] if teacher_outputs is not None else 0
    if cg:
    ...
    def get_logits(input_ids, inference_params):
        ...
        if not cg or not decoding:
            ...
            logits = model._decoding_cache.run(
                ...
                inference_params.seqlen_offset).squeeze(dim=1)
...
def update_graph_cache(
    ...
    if ((device, dtype) != (cache.device, cache.dtype)
        ...
        inf_cache = model.allocate_inference_cache(batch_size, max_seqlen,
                                                   dtype)
        lengths_per_sample = torch.full((batch_size, ),
                                        seqlen_og,
                                        dtype=torch.int32,
                                        device=device)
    ...
    def dispatch(input_ids, position_ids, seqlen):
        ...
        return cache.callables[batch_size,
                               decoding_seqlen](input_ids, position_ids,
                                                seqlen)
...
def capture_graph(model,
    ...
    input_ids = torch.full((batch_size, decoding_seqlen),
                           ...
                           dtype=torch.long,
                           device=device)
    position_ids = torch.full((batch_size, decoding_seqlen),
                              0,
                              dtype=torch.long,
                              device=device)
