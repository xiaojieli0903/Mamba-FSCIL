...
def layer_norm_ref(x,
    ...
    out = F.layer_norm(x.to(weight.dtype),
                       x.shape[-1:],
                       weight=weight,
                       bias=bias,
                       eps=eps).to(dtype)
...
def _layer_norm_fwd_1pass_kernel(
    ...
    if HAS_RESIDUAL:
        residual = tl.load(RESIDUAL + cols, mask=cols < N,
...
def _layer_norm_fwd(x,
    ...
    assert y.stride(-1) == 1
    if residual is not None or (residual_dtype is not None
                                and residual_dtype != x.dtype):
...
def _layer_norm_bwd(
    ...
    if bias is not None:
        ...
        assert bias.shape == (N, )
    # allocate output
    dx = (torch.empty_like(x) if x_dtype is None else torch.empty(
        M, N, dtype=x_dtype, device=x.device))
    ...
    if has_residual and dx.dtype == x.dtype:
        dresidual_in = dx
    return (dx, dw, db,
            dresidual_in) if not recompute_output else (dx, dw, db,
                                                        dresidual_in, y)
...
def layer_norm_fn(
    ...
    return LayerNormFn.apply(x, weight, bias, residual, eps, prenorm,
...
class LayerNormLinearFn(torch.autograd.Function):
    ...
    def forward(
        ...
        y, mean, rstd, residual_out = _layer_norm_fwd(
            ...
            eps,
            residual,
            out_dtype=None if not torch.is_autocast_enabled() else
